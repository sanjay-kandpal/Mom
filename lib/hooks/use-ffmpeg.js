...
import { setDownloadStatus as setWasmDownloadStatus } from "../services/wasm-checker";
...
export function useFFmpeg() {
  ...
  const downloadFFmpeg = useCallback(async (onProgress) => {
    try {
      ...
      // Clear sessionStorage download status on error so it can be retried
      setWasmDownloadStatus("idle");
      return false;
    }
  }, []);
